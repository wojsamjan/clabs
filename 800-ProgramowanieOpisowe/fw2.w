...
\getCVSdate$Date: 2004/09/30 16:47:35 $
\getCVSsource$Source: /var/cvs/wyklady/JP1-L/80-ProgramowanieOpisowe/fw2.w,v $
% --------------------------------------------------------------------
...
\def\title{FW}
\def\topofcontents{\null\vfill
  \centerline{\titlefont ZLICZANIE S£ÓW II}
  \vskip 15pt
...
@c
void wstaw_wierzcho³ek(struct wierzcho³ek **w, char *s)
{
  ...
  if (*w==NULL) 
    *w=nowy_wierzcho³ek(s);
    ...
    wstaw_wierzcho³ek(&((*w)->lewe_poddrzewo),s);
  else 
    wstaw_wierzcho³ek(&((*w)->prawe_poddrzewo),s);
}
...
int main () 
{
  ...
  while (getline(&bufor_p,&d³ugo¶æ_bufora,stdin)!=-1) {
    while ((nastêpne_s³owo=strtok(bufor_p,ogranicznik))!=NULL) {
      wstaw_wierzcho³ek(&korzeñ,lowercase(nastêpne_s³owo));
