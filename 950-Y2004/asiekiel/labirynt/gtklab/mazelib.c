...
void printMaze(void) {
  int i;
  for (i = 0; i < TheLabirynt.number_of_lines; ++i)
    printf("%s\n", TheLabirynt.lines[i].tab);
}

int mazeLines(void)
{
      return TheLabirynt.number_of_lines;
}

int mazeLineLength(int i)
{
      return TheLabirynt.lines[i].length;
}

int maxMazeWidth(void)
{
      int i, max = 0;

      for (i = 0; i < mazeLines(); ++i)
            if (mazeLineLength(i) > max)
                  max = mazeLineLength(i);
      return max;
}

void mazeCleanup(void)
{
      int i;
      for (i = 0; i < mazeLines(); ++i)
            free(TheLabirynt.lines[i].tab);
      free(TheLabirynt.lines);
}

void mazeOriginal(void)
{
      int i,j;
      pointT temp;
      ...
      for (i = 0; i < mazeLines(); ++i)
            for (j = 0; j < mazeLineLength(i); ++j) {
                  temp.x = j; temp.y = i;
                  if (IsMarked(temp))
                        SetTerrain(temp, TERRAIN_NORMAL);
            }
      SetTerrain(GetStartPosition(), TERRAIN_START_POS);
}
