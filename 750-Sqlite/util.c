...
int
drop_table (sqlite3 * db, const char *name)
{
  sqlite3_stmt *stmt;
...
void
print_error (sqlite3 * db, const char *msg, ...)
{
  char *txt;
...
int
print_sql_result (sqlite3 * db, const char *sql)
{
  char **result, *err;
...
int
print_stmt_result (sqlite3_stmt * stmt)
{
  int i, rc, ncols;
...
void
trace_sql (void *data, const char *sql)
{
  fprintf (stderr, "log:\t%s\n", sql);
}

void
log_sql (sqlite3 * db, int flag)
{
  if (flag != 0)
    {
...
/*  TODO: nie działa w UTF-8  */

const char *
convert_string (const char *value, int (*fn) (int))
{
  int i, len;
  const char *p;
  char *result;
...
const char *
lowercase (const char *value)
{
  return convert_string (value, tolower);
}

const char *
uppercase (const char *value)
{
